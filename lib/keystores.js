 ...
 *
 * @return {Promise<Array> | ExplainObject} The records that matched
 ...
 * @param {boolean} [options.explain=false] - An optional explain boolean.
 *
 * @return {Promise<Object> | ExplainObject} The database record or an
 ...
 * @param {boolean} [options.explain=false] - An optional explain boolean.
 *
 * @returns {Promise<object> | ExplainObject} The storage usage for the meter
...
exports.getStorageUsage = async ({
  ...
  while(cursor.hasNext()) {
    ...
    if(aggregate) {
      counters.push(aggregate({meterId, config, usage}));
    }
    ...
    counters.push(_addKeyCount({usage, kmsModule, keystoreId, signal}));
