...
exports.runOperation = async ({
  ...
  logger.debug('Running KMS operation.', {
    operation,
    storage,
  });
  let result;
  try {
    ...
    logger.error('runOperation Error', {error: e});
    const error = new BedrockError(
      'The KMS operation failed.', 'UnknownError', {
        httpStatusCode: 500,
        public: true,
      }, e);
    logger.error('The KMS operation failed.', {error});
    throw error;
  }
  return result;
};
